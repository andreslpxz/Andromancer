...
class AndroMancerAgent:
        ...
        while not self._stop_event.is_set() and self.mission.status == MissionStatus.RUNNING:
            ...
            try:
                ...
                if skill_override:
                    ...
                    # Execute skill plan
                    results = await self._execute_plan(skill_override.actions, observation)
                    ...
                    # 3. ACT
                    results = await self._execute_plan(thought.action_plan, observation)
    ...
    async def _execute_plan(self, actions: List[Dict], observation: Dict = None) -> List[ExecutionResult]:
        """Execute actions, supporting parallel execution if configured"""
        # Resolve SoM labels if present
        if observation and "label_map" in observation:
            label_map = observation["label_map"]
            for action in actions:
                ...
                if isinstance(element, dict) and "label" in element:
                    ...
                    if label in label_map:
                        params["element"] = label_map[label]
                        logger.info(f"Resolved SoM label {label} to element {params['element'].get('text') or params['element'].get('content_desc')}")
